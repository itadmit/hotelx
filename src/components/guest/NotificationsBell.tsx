...
export function GuestNotificationsBell({ hotelSlug, roomCode }: Props) {
  ...
  // Render the overlay into document.body via a portal. The guest layout uses
  // overflow:hidden on multiple ancestors, which would otherwise clip a
  // position:fixed drawer and make the Close button untappable.
  const overlay = open ? (
    <div
    ...
    >
      <button
        ...
        className="absolute inset-0 z-0 bg-ink/45 backdrop-blur-sm cursor-default border-0 p-0 appearance-none"
      ...
      >
        ...
        <button
          ...
          onClick={() => setOpen(false)}
          aria-label="Close"
          className="absolute top-2.5 right-2.5 z-20 h-10 w-10 rounded-full bg-background/95 border border-[color:var(--border)] shadow-sm flex items-center justify-center text-foreground/70 hover:text-ink hover:bg-surface transition-colors"
        >
